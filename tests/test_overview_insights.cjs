...
vm.runInContext(source.slice(source.indexOf('function renderInsights'), source.indexOf('function chartOptions')), context);
...
assert.match(nodes['insight-cards'].children[0].children[2].textContent, /50.0%/);
assert.match(nodes['insight-cards'].children[1].children[3].textContent, /เพียงประเภทวันเดียว/);
assert.match(nodes['insight-cards'].children[2].children[2].textContent, /เท่ากัน/);
assert.equal(nodes['insight-cards'].children[0].children[4].href, nodes['explore-map'].href);
...
context.renderInsights(data);
assert.match(nodes['insight-cards'].children[2].children[2].textContent, /ศูนย์/);
